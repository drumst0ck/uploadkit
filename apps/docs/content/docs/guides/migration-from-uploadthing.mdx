...
| `@uploadthing/next` | `@uploadkit/next` | Package rename |
| `@uploadthing/react` | `@uploadkit/react` | Package rename |
...
<Tabs groupId="migration" items={['Before (UploadThing)', 'After (UploadKit)']}>
  ...
  <Tab value="After (UploadKit)">
    ```tsx
    import { UploadButton, UploadDropzone } from '@uploadkit/react';
...
**Open-source SDK** — `@uploadkit/core`, `@uploadkit/react`, and `@uploadkit/next` are all MIT licensed and published on npm.

<Callout type="info">
  Need help with your migration? Open a [GitHub discussion](https://github.com/uploadkit/uploadkit/discussions) or reach out at support@uploadkit.dev.
</Callout>
