...
The API key is **server-side only** — it is never exposed to the browser. The `@uploadkit/next` handler uses this key to authenticate with the UploadKit API. React components communicate through your local `/api/uploadkit` endpoint without needing the key.
