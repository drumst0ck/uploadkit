...
The `NEXT_PUBLIC_` prefix exposes the key to the browser for direct SDK use. This is safe because UploadKit API keys are scoped to your project and rate-limited — treat them like public access tokens, not passwords.

For server-side only use (e.g., file deletion, listing files via the REST API), use only the non-public `UPLOADKIT_API_KEY`.
