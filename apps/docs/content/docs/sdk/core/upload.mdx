...
description: Basic uploads, progress tracking, metadata, abort, and error handling with @uploadkit/core.
---
...
`client.upload()` throws an `UploadKitError` on failure. Import it from `@uploadkit/core` for typed error handling.
