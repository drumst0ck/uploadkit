...
If you are using BYOS mode with your own S3/R2 bucket, configure your storage credentials in the server-side `@uploadkitdev/next` handler instead — the core client works identically from the browser regardless of storage mode. See the [BYOS guide](/docs/core-concepts/byos) for details.
