...
**Self-hosted BYOS** — Use your own S3 or Cloudflare R2 bucket with the open-source SDK (`storage` in `createUploadKitHandler`). Your files never touch UploadKit Cloud storage. The dashboard is for managed Cloud projects only.
