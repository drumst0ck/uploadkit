...
<Callout type="info">
  Get your API key from [app.uploadkit.dev](https://app.uploadkit.dev). Never prefix this with `NEXT_PUBLIC_` — the key is used only by the server-side route handler, never by client components.
</Callout>
