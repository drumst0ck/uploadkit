...
import { createUploadKitHandler } from '@uploadkit/core/server';
...
For maximum control, you can use `createProxyClient` from `@uploadkit/core` without any React dependency. The proxy client talks to your local backend endpoint (which holds the API key) rather than the UploadKit API directly:

```typescript title="src/lib/upload.ts"
import { createProxyClient } from '@uploadkit/core';
