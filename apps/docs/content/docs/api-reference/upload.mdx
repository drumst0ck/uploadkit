...
<Callout type="info">
  When using the `@uploadkit/core` or `@uploadkit/react` SDK, multipart uploads are handled automatically for files over 10 MB. You do not need to call these endpoints manually.
</Callout>
