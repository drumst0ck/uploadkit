...
2. Your server ──► Storage        CreatePresignedPost / PutObject
              (URL expires in 15 minutes for single PUT, 1 hour for multipart parts)
...
Presigned URLs expire after **15 minutes for single-PUT uploads** and **1 hour for multipart part URLs**. After expiry, the URL is rejected by the storage provider regardless of the signature. This limits the window during which a leaked URL could be exploited.
