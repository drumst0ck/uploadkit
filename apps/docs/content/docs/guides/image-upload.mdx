...
<Tabs groupId="framework" items={['Next.js', 'React (API key)']}>
  ...
  <Tab value="React (API key)">
    ```typescript
    // No file router needed — validation happens in the UploadKit API
    // Set NEXT_PUBLIC_UPLOADKIT_API_KEY in your environment
    ```

    When using the React SDK directly with an API key, file type and size limits are configured in your project's file routes in the [dashboard](/docs/dashboard/overview).
  </Tab>
</Tabs>
...
import { UploadDropzone, FilePreview } from '@uploadkit/react';
...
For in-progress thumbnail previews (before the upload completes), use the `FilePreview` component from `@uploadkit/react` — it generates thumbnails client-side via canvas.
