...
Install both `@uploadkit/next` for the server-side handler and `@uploadkit/react` for UI components:
...
# Server-side: used by createUploadKitHandler to authenticate with the UploadKit API
UPLOADKIT_API_KEY=uk_live_xxxxxxxxxxxxxxxxxxxxx

# Client-side: used by UploadKitProvider
NEXT_PUBLIC_UPLOADKIT_API_KEY=uk_live_xxxxxxxxxxxxxxxxxxxxx
```

<Callout type="warn">
  Never expose `UPLOADKIT_API_KEY` (without `NEXT_PUBLIC_`) in client-side code. If using BYOS, your storage credentials go here too — server-side only.
</Callout>

## Add the provider

Wrap your root layout's `children` with `UploadKitProvider` so all components in the tree can access the upload client:
