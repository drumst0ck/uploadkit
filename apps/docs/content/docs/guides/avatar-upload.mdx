...
export function CustomAvatarUpload({ onSuccess }: { onSuccess: (url: string) => void }) {
  ...
  const { upload, isUploading, progress, result } = useUploadKit('avatarUploader');
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await upload(file);
    // `result` will populate once the upload succeeds — consume it via useEffect
    // or by reading `result` directly in render.
    if (result) onSuccess(result.url);
  };
