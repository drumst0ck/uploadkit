...
If you want to skip ahead, [UploadKit](https://docs.uploadkit.dev/docs/sdk/react) ships a `<Cropper>` component that does all of this with a themed design system, dark mode, keyboard support, and a touch-friendly variant. But understanding the underlying mechanics will make you a better consumer of any cropping library.
...
Or use [UploadKit's `<Cropper>`](https://docs.uploadkit.dev/docs/sdk/react) which handles all of the above plus dark mode, theming via CSS custom properties, and integration with the upload pipeline so the cropped blob goes straight to storage with no extra glue:
