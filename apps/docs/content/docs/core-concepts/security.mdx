...
```tsx title="app/layout.tsx — correct"
...
Every request to the UploadKit API must include a valid API key as a Bearer token in the `Authorization` header. UploadKit issues two types of keys:
...
| Expiry | Limits the window for URL leakage — 15 minutes for single PUTs, 1 hour for multipart parts |
...
UploadKit applies a fixed-window rate limit (60-second window) per API key, backed by Redis with an in-memory fallback:

| Endpoint group | Limit |
|---------------|-------|
| Upload endpoints (`/upload/*`) | 30 req/min |
| All other endpoints | 10 req/min |

When the limit is exceeded, UploadKit returns `429 Too Many Requests` with an error message containing the retry delay. The client SDK automatically retries with exponential backoff up to `maxRetries` attempts.

## Tier limits

In addition to per-request rate limits, each subscription tier enforces cumulative storage and upload-count limits per billing period:

| Limit | Free | Pro | Team | Enterprise |
|-------|------|-----|------|------------|
| Storage | 5 GB | 100 GB | 1 TB | Unlimited |
| Bandwidth | 2 GB | 200 GB | 2 TB | Unlimited |
| Monthly uploads | 1,000 | 50,000 | 500,000 | Unlimited |
| Max file size | 4 MB | 512 MB | 5 GB | 10 GB |
| Max projects | 2 | 10 | 50 | Unlimited |
| Max API keys per project | 2 | 10 | 50 | Unlimited |
| Image transforms/month | — | 5,000 | 25,000 | From 100,000 |

Exceeding a storage or upload-count limit on the **Free** plan blocks further uploads until the next billing period. On **Pro** and above, usage is metered and billed as overage via Stripe — uploads continue uninterrupted.
