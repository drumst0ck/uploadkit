...
<Callout type="info">
  When using the `@uploadkit/core` or `@uploadkit/react` SDK, errors are surfaced via the `onError` callback with the same `code` values. You do not need to parse the HTTP response manually.
</Callout>
